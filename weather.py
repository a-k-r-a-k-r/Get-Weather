...
root.iconbitmap('resources/icons/weather.ico')
...
#Define functions
def search():
    """Use open weather api to look up current weather conditions, given a city/ zipcode, country"""
    global response

    #Get API response
    #URL and my api key....USE YOUR OWN API KEY!
    url = 'https://api.openweathermap.org/data/2.5/weather'
    api_key = 'Replace this with your API Key' #USE YOUR OWN API KEY
    ...
    '''{'coord': {'lon': -71.06, 'lat': 42.36}, 'weather': [{'id': 500, 'main': 'Rain',
...
def get_weather():
    try:
        ...
        city_info_label.config(text="Unable to find details",font=large_font, bg=output_color)
        weather_label.config(text="Check whether you entered the correct details",font=small_font, bg=output_color)
        temp_label.config(text="Make sure that you are connected to internet", font=small_font,bg=output_color)
        feels_label.config(text="Make sure that you are using your-own API")
        temp_min_label.config(text=" ", font=small_font,bg=output_color)
        temp_max_label.config(text=" ", font=small_font,bg=output_color)
        humidity_label.config(text=" ", font=small_font, bg=output_color)
